  ...
  <n-modal-provider>
    <n-modal v-model:show="showModal" :mask-closable="false">
      ...
      >
        <n-form ref="formRef" inline label-width="100%" :model="formValue" :rules="rules">
          <div class="forget-main-box">
            <div class="steps-box">
              ...
              <van-steps :active="current">
                <van-step>身份验证</van-step>
                <van-step>密码重置</van-step>
                <van-step>重置完成</van-step>
              </van-steps>
            </div>
            <div class="form-content-box">
              <div class="content-input-box" v-show="current === 0">
              ...
              <div class="content-input-box finsh-input" v-show="current === 2">
                重置完成, 去登录！
              </div>
            </div>
          </div>
        ...
        <template #footer>
          <n-button type="info" @click="onNegativeClick" strong secondary v-show="current !== 2">
            取消
          </n-button>
          <n-button type="info" @click="prev" v-show="current == 1"> 上一步 </n-button>
          <n-button type="info" @click="next" v-show="current == 0 || current == 1">
            下一步
          </n-button>
          <n-button type="info" @click="onNegativeClick" v-show="current === 2"> 完成 </n-button>
        </template>
      </n-card>
    </n-modal>
  </n-modal-provider>
...
const current = ref<string | number | undefined>(0)
...
// 上一步
const prev = () => {
  if (current.value === 0) current.value = undefined
  else if (current.value === undefined) current.value = 2
  else current.value = Number(current.value) - 1
}
// 关闭
